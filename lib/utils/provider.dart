...
class Provider {
  ...
  Future init(bool isCreate) async {
    ...
    if (!tableIsRight) {//不存在 或者表结构异常    // Should happen only the first time you launch your applicatio
      // 关闭上面打开的db，否则无法执行open
      db.close();
      // Delete the database
      await deleteDatabase(path);
      // Copy from asset
      ByteData data = await rootBundle.load(join("assets", "app.db"));//获取assets下 app.adb数据库的数据
      List<int> bytes =
          data.buffer.asUint8List(data.offsetInBytes, data.lengthInBytes);//z转换为bytes
      await new File(path).writeAsBytes(bytes);//存储到 本地数据库 flutter.db  // Write and flush the bytes written
// open the database
      db = await openDatabase(path, version: 1,
          onCreate: (Database db, int version) async {
        print('db created version is $version');//db 创建成功
      }, onOpen: (Database db) async {
        print('new db opened');//打开新建的db
      });
    } else {
      print("Opening existing database");//第二次打开app 表存在
    }
  }
